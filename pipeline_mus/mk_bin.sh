...
observable="dis len run max chg net abd_log gvy pip iac mfc csl tsl ssa ssb ssc pol tmx toh tom sst phs mag oxi rbp pfm ppf coc"
count="pap_cnt plc_cnt rbp_cnt prW_cnt"  # MUST BE ADDED
aas="R H K D E S T N Q C U G P A V I L M F Y W"
mus_exp="young_bin old_bin"
age="young old"
yng="young_bin"
old="old_bin"
col_list="\"slateblue\", \"tomato\", \"gainsboro\""
...
plot_parms () {
  ...
  if [ $1 == "iac" ]; then echo "Biogrid_Interactions 0 25"; fi
  ...
  if [ $1 == "tom" ]; then echo "N_Turnover 0 200"; fi
...
## UNIVERSAL VARIABLES ##
mycol=c(\"slateblue\",\"tomato\",\"gainsboro\")"
echo -n "mylevmus=c("
for lvl in $age; do 
  echo -n "\"$lvl\"," 
done
...
echo "#
my.mus <- my.mus[!duplicated(my.mus),]
my.aam <- my.aam[!duplicated(my.aam),]"
...
for obs in $count; do
  ...
  for exp in $age; do var="${var}\"$exp\","; done
...
for obs in $observable; do
  ...
  var="${var}wilcox.test(my.mus[which(my.mus\$tag=='young'),]\$$obs, my.mus[which(my.mus\$tag=='old'),]\$$obs)\$p.val)"; #var="${var%,)})"
  echo "$var"
done
...
for obs in $count; do 
  ...
  hit_sample="length(my.mus[which(my.mus\$tag==\"young\" & my.mus\$$obs==1),]\$nam)"
  hit_left="(length(my.mus[which(my.mus\$tag==\"old\" & my.mus\$$obs==1),]\$nam))"
  miss_sample="length(my.mus[which(my.mus\$tag==\"young\" & my.mus\$$obs!=1),]\$nam)"
  miss_left="(length(my.mus[which(my.mus\$tag==\"old\" & my.mus\$$obs!=1),]\$nam))"
  my_test="fisher.test(matrix(c($hit_sample,$hit_left,$miss_sample,$miss_left),2,2),alternative='two.sided')\$p.value"
  var="${var}$my_test)"
  echo "$var"
done
...
echo "#
young <- p.adjust(p_mus[,1],method=\"hochberg\")
old <- p.adjust(p_mus[,2],method=\"hochberg\")
y2o <- p.adjust(p_mus[,3],method=\"hochberg\")
corr_pval<-cbind(young,old,y2o)
